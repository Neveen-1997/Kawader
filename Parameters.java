...
//********************************************************************************************************************************************************************************************************//
	...
	public void AddJobWithFilter() throws InterruptedException, IOException {
		...
		WebElement AddJobBtn = wait
				.until(ExpectedConditions.elementToBeClickable(By.xpath("//span[contains(text(),'إضافة وظيفة')]")));
		AddJobBtn.click();

		WebElement DateField = wait.until(
				ExpectedConditions.elementToBeClickable(By.xpath("//input[@placeholder='تاريخ انتهاء الإعلان']")));
		DateField.click();
		DateField.sendKeys(Keys.ENTER);

		WebElement PositionField = wait.until(
				ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@placeholder='المسمى الوظيفي']")));
		PositionField.sendKeys("محاسب");
		wait.until(ExpectedConditions.textToBePresentInElementValue(PositionField, "محاسب"));
		...
		new Select(ConTypeField).selectByValue("4");

		WebElement DurationField = driver.findElement(By.xpath("//input[@placeholder='مدة التعاقد']"));
		DurationField.sendKeys("2");

		WebElement PlaceField = driver.findElement(By.xpath("//div[11]//select[1]"));
		new Select(PlaceField).selectByValue("4");

		WebElement SalaryField = driver.findElement(By.xpath("//input[contains(@placeholder,'الراتب')]"));
		SalaryField.sendKeys("2000");
		wait.until(ExpectedConditions.textToBePresentInElementValue(SalaryField, "2000"));

		WebElement OverTimeField = driver.findElement(By.xpath("//input[contains(@placeholder,'المكافأة')]"));
		OverTimeField.sendKeys("100");
		wait.until(ExpectedConditions.textToBePresentInElementValue(OverTimeField, "100"));
		...
		WebElement AddSubTaskBtn = wait.until(
				ExpectedConditions.elementToBeClickable(By.xpath("//button[contains(text(),'إضافة مهمة فرعية')]")));
		AddSubTaskBtn.click();

		WebElement SubTasksField = wait.until(ExpectedConditions.visibilityOfElementLocated(
				By.xpath("//div//div//div//div//div[2]//div[1]//div[1]//div[1]//input[1]")));
		SubTasksField.sendKeys("تطوير التصاميم وتحسينها");

		WebElement SaveBtn = wait
				.until(ExpectedConditions.elementToBeClickable(By.xpath("//button[contains(text(),'حفظ وإرسال')]")));
		SaveBtn.click();

		wait.until(ExpectedConditions.urlToBe("https://kawader.amyalsmart.com/board/companies/1/companiesManagement"));

		System.out.println("the job added successfully");
	}
	...
	public void SaveDraftBtn() throws IOException {
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));
		...
		WebElement SaveDraftBtn = wait
				.until(ExpectedConditions.elementToBeClickable(By.cssSelector("button[aria-label='حفظ كنموذج']")));
		SaveDraftBtn.click();

		driver.navigate().back();
		wait.until(ExpectedConditions.urlToBe("https://kawader.amyalsmart.com/board/companies/1/companiesManagement"));

		WebElement AddJobBtn2 = wait
				.until(ExpectedConditions.elementToBeClickable(By.xpath("//span[contains(text(),'إضافة وظيفة')]")));
		AddJobBtn2.click();

		WebElement PositionField2 = wait.until(
				ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@placeholder='المسمى الوظيفي']")));

		wait.until(ExpectedConditions.textToBePresentInElementValue(PositionField2, "مبرمج فرونت اند"));
...
//		assertTrue(StudentActual, "Can't find the student in the required for interview section");
	...
	public void AddUser() throws InterruptedException {
		...
		WebElement UsersBtn = wait.until(ExpectedConditions.visibilityOfElementLocated(By.linkText("المستخدمين")));

		UsersBtn.click();

		WebElement AddUsersBtn = wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector(
				"body > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > div:nth-child(2) > div:nth-child(2) > div:nth-child(1) > div:nth-child(1) > div:nth-child(2) > button:nth-child(1)")));

		AddUsersBtn.click();

		WebElement Name = driver.findElement(By.cssSelector("input[placeholder='الأسم ']"));

		Name.sendKeys("try adding user automaticly");

		WebElement JobID = driver.findElement(By.cssSelector("input[placeholder='الرقم الوظيفي ']"));

		JobID.sendKeys("1997");

		WebElement ID = driver.findElement(By.cssSelector("input[placeholder='رقم الهوية ']"));

		ID.sendKeys("2536021377");

		WebElement Position = driver.findElement(By.cssSelector("input[placeholder='المسمى الوظيفي ']"));

		Position.sendKeys("Hr Manager");

		WebElement Email = driver.findElement(By.cssSelector("input[placeholder='البريد الالكتروني']"));

		Email.sendKeys("HrManager@roayasmart.com");

		WebElement PhoneNumber = driver.findElement(By.cssSelector("input[placeholder='رقم الجوال']"));

		PhoneNumber.sendKeys("0531342278");

		WebElement Pass = driver.findElement(By.id("password"));

		Pass.sendKeys("0531342278");

		WebElement PassCon = driver.findElement(By.id("password_confirmation"));

		PassCon.sendKeys("0531342278");

		WebElement PositionName = driver.findElement(By.cssSelector("input[placeholder='إسم الصلاحية']"));

		PositionName.sendKeys("Hr Manager");

		WebElement UserType = driver.findElement(By.cssSelector(
				".MuiInputBase-root.MuiOutlinedInput-root.MuiInputBase-colorPrimary.MuiInputBase-fullWidth.MuiInputBase-formControl.MuiInputBase-adornedEnd.MuiAutocomplete-inputRoot.muirtl-14kjrss"));

		UserType.click();

		JavascriptExecutor js = (JavascriptExecutor) driver;

		js.executeScript("arguments[0].value='ادمن';", UserType);

		String enteredValue1 = UserType.getAttribute("value");

		System.out.println("Entered Value in user type: " + enteredValue1);
